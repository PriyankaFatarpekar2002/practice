...
import { format, parseISO, startOfWeek, eachDayOfInterval, isSameDay, addDays } from 'date-fns';
...
const ListView = () => {
  ...
  const [currentWeek, setCurrentWeek] = useState(new Date('2025-07-01')); // Start from July 1st
  ...
  const getWeekRange = (date) => {
    // Start from July 1st and show 6 days
    const start = new Date(date);
    const end = addDays(start, 5); // July 1st + 5 days = July 6th
    return { start, end };
  };
  ...
  const handleSaveEdit = async () => {
    if (editingEntry) {
      ...
      try {
        const entryRef = doc(db, "AddNewEntry", editForm.id);
  ...
  const { start, end } = getWeekRange(currentWeek);
  const weekRangeText = `${format(start, 'dd')} - ${format(end, 'dd MMMM')}, ${format(end, 'yyyy')}`;
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 bg-white font-sans border border-gray-200 shadow-md rounded-2xl"style={{ fontFamily: 'Inter' }}>
      <div className="flex flex-col sm:flex-row justify-between items-start mb-6 sm:mb-8 gap-4">
        <div>
          ...
          <p className="text-[#6B7280] text-sm font-[400]">{weekRangeText}</p>
        </div>
                  ...
                  {editingEntry?.dayIndex === dayIndex && editingEntry?.entryIndex === entryIndex ? (
                    ...
                    <div className="bg-white border border-[#E5E7EB] rounded-lg p-3 hover:shadow-sm transition-shadow relative">
                      ...
                      {/* <div className="hidden sm:block mt-1">
                        <span className="text-[#9CA3AF] font-[400] text-xs">{entry.workType}</span>
                      </div> */}
                    </div>
                  )}
                </div>
